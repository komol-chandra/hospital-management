    ...
    <div class="col-sm-12">
        <div class="panel panel-bd lobidrag">
            <div class="panel-heading">
                <div class="btn-group"> 
                    <button type="button" class="btn btn-success m-r-2 m-b-5" data-toggle="modal" data-target="#modal-success"><i class="fa fa-plus"></i> Add New  Payment</button>                      
                </div>        
            </div>
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $value->account->name ?? null}}</td>
                                <td>{{ $value->name ?? null}}</td>
                                <td>{{ $value->pay_to ?? null}}</td>
                                ...
                                <td>
                                    @if($value->status == 1)
                                    <a class="btn btn-danger btn-xs" id="status" href="/admin/payment/status/{{ $value->id }}"><i class="fa fa-refresh"></i></a>
                                    @else
                                    <a class="btn btn-info btn-xs" id="status" href="/admin/payment/status/{{ $value->id }}"><i class="fa fa-refresh"></i></a>
                                    @endif
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h1 class="modal-title">Add payment</h1>
            </div>
            <div class="modal-body">
            {!! Form::open(['url' => '/admin/payment','method'=>'post','files'=>true,'id'=>'form_insert']) !!}
            @include('backend.pages.payment.form');
            </div>
...
<div class="modal fade modal-success" id="modal-edit" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h1 class="modal-title">Update payment</h1>
            </div>
            <div class="modal-body">
            {!! Form::open(['url' => '/admin/payment/'.$value->id.'','method'=>'put','files'=>true,'id'=>'form_update']) !!}
    ...
    iziToast.question({
        ...
        buttons: [
            ['<button><b>YES</b></button>', function () {
                ...
                $form.attr('action','/admin/payment/'+id);
                $form.submit()
            }, true],
...
<script src="{{asset('backend/script/payment.js')}}"></script>
@endsection